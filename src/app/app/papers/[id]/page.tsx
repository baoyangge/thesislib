...
export default async function PaperDetailsPage({
  ...
  // Wait, the prompt says "只有管理员才可以编辑" (Only admins can edit). We should show an edit button if user is admin.
  // We don't necessarily need to implement the edit page if it's already there or not explicitly requested, but let's add the button.
  // Or link to an edit page if there's one.
      ...
      <main className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12">
        <article className="bg-white border border-slate-200 rounded-lg p-8 shadow-sm">
          ...
          <div className="mb-8 border-t border-slate-200 pt-6">
            <h3 className="text-xl font-semibold text-slate-900 mb-3">Abstract</h3>
          ...
          <div className="flex items-center gap-4 border-t border-slate-200 pt-6">
            ...
            {canEdit && (
              <Link href={`/admin/papers/${paper.id}/edit`} className="inline-flex items-center px-6 py-3 border border-slate-300 text-base font-medium rounded-md text-slate-700 bg-white hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">
