    ...
    <div className="min-h-screen bg-white text-slate-900 font-sans">
      ...
      >
        ...
        <div className="relative z-10">
          <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-6 drop-shadow-md">{t("hero_title")}</h1>
          <p className="text-xl text-blue-50 mb-10 max-w-2xl mx-auto drop-shadow">{t("hero_subtitle")}</p>
              ...
              {topPapers.map((p) => (
                <div key={p.id} className="flex flex-col bg-white border border-slate-200 rounded-lg shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                  ...
                  <div className="bg-slate-50 px-6 py-4 border-t border-slate-100">
                    <Link href={`/app/papers/${p.id}`} className="text-blue-700 font-medium hover:text-blue-900 flex items-center text-sm">
                      <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>{t("details") || "Details"}</Link>
                  </div>
                </div>
              ))}
            </div>
